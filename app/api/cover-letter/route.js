...
async function proxy(req) {
  ...
  if (
    process.env.COVER_LETTER_AUTH_HEADER_KEY &&
    process.env.COVER_LETTER_AUTH_HEADER_VALUE
  ) {
    headers[process.env.COVER_LETTER_AUTH_HEADER_KEY] =
      process.env.COVER_LETTER_AUTH_HEADER_VALUE;
  }
  ...
  try {
    ...
    return NextResponse.json(
      { statusFromWebhook: res.status, data },
      { status: res.status }
    );
  } catch (e) {
    const cause = e.cause || {};
    return NextResponse.json(
      {
        error: "Fetch to Cover Letter webhook failed",
        detail: e.message,
